...
int main(int argc, char *argv[]) {
    ...
    FILE *arquivo_saida = fopen("edf_chccp.out", "w");
    ...
    fprintf(arquivo_saida, "EXECUTION BY EDF\n\n");
        ...
        if (t > 0) {
            for (int i = 0; i < total_tarefas; i++) {
                if (t % tasks[i].periodo == 0) {
                    ...
                    tasks[i].proximo_deadline = t + tasks[i].periodo;
                }
            }
        }

        int proxima_tarefa = -1;
        int menor_deadline = 999999; 
        
        for (int i = 0; i < total_tarefas; i++) {
            if (tasks[i].tempo_restante > 0 && tasks[i].proximo_deadline < menor_deadline) {
                proxima_tarefa = i;
                menor_deadline = tasks[i].proximo_deadline;
            }
        }
